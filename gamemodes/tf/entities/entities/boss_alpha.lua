...
ENT.Spawnable		= false
ENT.Model = "models/bots/knight/knight.mdl"
ENT.AttackDelay = 50
ENT.AttackDamage = 30
ENT.AttackRange = 800
...
function ENT:Initialize()
	...
	if SERVER then
		local larm = ents.Create("gmod_button")
		larm:SetModel("models/bots/knight/armor_arm_upper_l.mdl")
		larm:SetPos(self:GetPos())
		larm:SetAngles(self:GetAngles())
		larm:SetParent(self)
		larm:AddEffects(bit.bor(EF_BONEMERGE,EF_BONEMERGE_FASTCULL))
		local rarm = ents.Create("gmod_button")
		rarm:SetModel("models/bots/knight/armor_arm_upper_r.mdl")
		rarm:SetPos(self:GetPos())
		rarm:SetAngles(self:GetAngles())
		rarm:SetParent(self)
		rarm:AddEffects(bit.bor(EF_BONEMERGE,EF_BONEMERGE_FASTCULL))
		local lleg = ents.Create("gmod_button")
		lleg:SetModel("models/bots/knight/armor_leg_upper_l.mdl")
		lleg:SetPos(self:GetPos())
		lleg:SetAngles(self:GetAngles())
		lleg:SetParent(self)
		lleg:AddEffects(bit.bor(EF_BONEMERGE,EF_BONEMERGE_FASTCULL))
		local rleg = ents.Create("gmod_button")
		rleg:SetModel("models/bots/knight/armor_leg_upper_r.mdl")
		rleg:SetPos(self:GetPos())
		rleg:SetAngles(self:GetAngles())
		rleg:SetParent(self)
		rleg:AddEffects(bit.bor(EF_BONEMERGE,EF_BONEMERGE_FASTCULL))
		self:SetNWEntity("LeftArm",larm)
		self:SetNWEntity("RightArm",rarm)
		self:SetNWEntity("LeftLeg",lleg)
		self:SetNWEntity("RightLeg",rleg)
	end
	local seq = "spawn"
...
function ENT:BodyUpdate()
	...
	--
	if ( act == ACT_MP_RUN_MELEE || act == self:GetSequenceActivity(self:LookupSequence("run_secondary")) ) then
