...
PrecacheParticleSystem( "repair_claw_heal_blue" )
PrecacheParticleSystem( "repair_claw_heal_red" )
...
ENT.Model = "models/weapons/w_models/w_repair_claw.mdl"
...
local function ArrowBulletCallback(att, tr, dmginfo)
	...
	if ent:IsSolid() and !ent:IsNPC() or !ent:IsPlayer() then
		...
		util.Effect("tf_arrow_stuck_rescue_building", effectdata)

		inf:EmitSound(ImpactSounds[tr.MatType] or "Weapon_Arrow.ImpactMetal")
	elseif ent:IsPlayer() or ent:IsNPC() or ent:IsBuilding() then
...
function ENT:Hit(ent)
	...
	if ent:IsBuilding() and ent:IsFriendly(owner) then
		GAMEMODE:HealPlayer(owner, ent, Lerp(fraction, self.RangedMinHealing, self.RangedMaxHealing), true, false)
		if GAMEMODE:EntityTeam(self:GetOwner()) == TEAM_BLU then
			ParticleEffect( "repair_claw_heal_blue", self:GetPos(), self:GetAngles() )	
		elseif GAMEMODE:EntityTeam(self:GetOwner()) == TF_TEAM_PVE_INVADERS then
			ParticleEffect( "repair_claw_heal_blue", self:GetPos(), self:GetAngles() )	
		else
			ParticleEffect( "repair_claw_heal_red", self:GetPos(), self:GetAngles() )	
		end
		self:EmitSound("Weapon_Arrow.ImpactMetal")
...
hook.Add("PostScaleDamage", "TFArrowBuildingPinRegister", function(ent, hitgroup, dmginfo)
	local inf = dmginfo:GetInflictor()
	
	if inf:GetClass()=="tf_projectile_arrow_heal_building" and inf.HitPos then
