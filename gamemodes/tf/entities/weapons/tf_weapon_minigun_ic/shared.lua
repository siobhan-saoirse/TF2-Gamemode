...
sound.Add(
{
name = "Weapon_IronCurtain.ShootLoop",
channel = "CHAN_AUTO",
pitch = 95,
volume = "VOL_NORM",
level = 95,
sound = ")weapons/iron_curtain_shoot.wav"
} )
sound.Add(
{
name = "Weapon_IronCurtain.ShootLoopCrit",
channel = "CHAN_AUTO",
pitch = 95,
volume = "VOL_NORM",
level = 95,
sound = ")weapons/iron_curtain_shoot_crit.wav"
} )
sound.Add(
{
name = "Weapon_IronCurtain.SpinUp",
channel = "CHAN_AUTO",
pitch = 95,
volume = "VOL_NORM",
level = 95,
sound = ")weapons/iron_curtain_wind_up.wav"
} )
sound.Add(
{
name = "Weapon_IronCurtain.SpinDown",
channel = "CHAN_AUTO",
pitch = 95,
volume = "VOL_NORM",
level = 95,
sound = ")weapons/iron_curtain_wind_down.wav"
} )
...
SWEP.WorldModel			= "models/weapons/c_models/c_minigun/c_minigun_natascha.mdl"
...
SWEP.ReloadSound = Sound("Weapon_Minifun.Reload")
SWEP.EmptySound = Sound("Weapon_Minifun.ClipEmpty")
SWEP.ShootSound2 = Sound("Weapon_IronCurtain.ShootLoop")
SWEP.SpecialSound1 = Sound("Weapon_IronCurtain.SpinUp")
SWEP.SpecialSound2 = Sound("Weapon_IronCurtain.SpinDown")
SWEP.SpecialSound3 = Sound("Weapon_Minigun.Spin")
SWEP.ShootCritSound = Sound("Weapon_IronCurtain.ShootLoopCrit")
...
function SWEP:PrimaryAttack(vampire)
	...
	if self:RollCritical() then
		...
		if self.Critting or not self.Firing then
			...
			self.SpinSound:Stop()
			self.ShootCritSoundLoop:Stop()
			self.ShootSoundLoop:Play( "Weapon_Tomislav.ShootLoop", self:GetOwner():GetPos(), 95, 95, "VOL_NORM")
			if self.Primary.Delay == 0.06 then
	...
	if ( CLIENT ) then
		...
		bone = self.CModel:LookupBone("barrel")
			if bone then
				...
				:ManipulateBoneAngles( bone, Angle(0,self.barrelRotation,0) )
			else
				return
			end
			else
