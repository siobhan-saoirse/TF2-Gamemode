	...
	if (self:GetItemData()) then
		if (self:GetItemData().model_player) then
			...
			if (self:GetItemData().model_player == "models/workshop/weapons/c_models/c_demo_sultan_sword/c_demo_sultan_sword.mdl") then
				...
				self.VM_DRAW			= _G["ACT_VM_DRAW_SPECIAL"]
				self.VM_IDLE			= _G["ACT_VM_IDLE_SPECIAL"]
				self.VM_HITCENTER	= _G["ACT_VM_HITCENTER_SPECIAL"]
				self.VM_SWINGHARD	= _G["ACT_VM_SWINGHARD_SPECIAL"]
				self.VM_PRIMARYATTACK	= _G["ACT_VM_PRIMARYATTACK_SPECIAL"]
				self.VM_SECONDARYATTACK	= _G["ACT_VM_SECONDARYATTACK_SPECIAL"]
				self.VM_RELOAD			= _G["ACT_VM_RELOAD_SPECIAL"]
				self.VM_RELOAD_START	= _G["ACT_RELOAD_START_SPECIAL"]
				self.VM_RELOAD_FINISH	= _G["ACT_RELOAD_FINISH_SPECIAL"]
				
				-- Special activities
				self.VM_CHARGE			= _G["ACT_VM_CHARGE_SPECIAL"]
				self.VM_DRYFIRE			= _G["ACT_VM_DRYFIRE_SPECIAL"]
				self.VM_IDLE_2			= _G["ACT_VM_IDLE_2_SPECIAL"]
				self.VM_CHARGE_IDLE_3	= _G["ACT_VM_CHARGE_IDLE_3_SPECIAL"]
				self.VM_IDLE_3			= _G["ACT_VM_IDLE_3_SPECIAL"]
				self.VM_PULLBACK		= _G["ACT_VM_PULLBACK_SPECIAL"]
				self.VM_PREFIRE			= _G["ACT_ATTACK_STAND_PREFIRE_SPECIAL"]
				self.VM_POSTFIRE		= _G["ACT_ATTACK_STAND_POSTFIRE_SPECIAL"]
				
				self.VM_INSPECT_START	= _G["ACT_VM_INSPECT_START_SPECIAL"]
				self.VM_INSPECT_IDLE	= _G["ACT_VM_INSPECT_IDLE_SPECIAL"]
				self.VM_INSPECT_GND		= _G["ACT_VM_INSPECT_GND_SPECIAL"]
			end
