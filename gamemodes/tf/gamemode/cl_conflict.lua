for k, v in pairs(engine.GetAddons()) do 
	...
	if v.wsid == "347309307" and v.mounted == true then
		...
		function conflictbut.DoClick()
			steamworks.ViewFile(347309307)
		end
	elseif v.wsid == "423461816" and v.mounted == true then
		if not game.SinglePlayer() then if not LocalPlayer():IsListenServerHost() then return end end
		local conflict_help_frame = vgui.Create( "DFrame" )
		conflict_help_frame:SetSize(200, 200)
		conflict_help_frame:Center()
		...
		conflicttext:CenterHorizontal(0.5)
		conflicttext:SetVerticalScrollbarEnabled(false)
		conflicttext:AppendText("A addon named "..v.title.." (id "..v.wsid..") has been detected. The addon is conflicting with many core features and other things that is part of Team Fortress 2 Gamemode. Please uninstall/disable the addon and reload the map.")
		...
		function conflictbut.DoClick()
			steamworks.ViewFile(423461816)
		end
	elseif v.wsid == "949733637" and v.mounted == true then
		if not game.SinglePlayer() then if not LocalPlayer():IsListenServerHost() then return end end
		local conflict_help_frame = vgui.Create( "DFrame" )
		conflict_help_frame:SetSize(200, 200)
		conflict_help_frame:Center()
		...
		function conflictbut.DoClick()
			steamworks.ViewFile(949733637)
		end
	elseif v.wsid == "174375624" and v.mounted == true then
		if not game.SinglePlayer() then if not LocalPlayer():IsListenServerHost() then return end end
		local conflict_help_frame = vgui.Create( "DFrame" )
		conflict_help_frame:SetSize(200, 200)
		conflict_help_frame:Center()
		...
		function conflictbut.DoClick()
			steamworks.ViewFile(174375624)
		end
	elseif v.wsid == "2828787576" and v.mounted == true then
		if not game.SinglePlayer() then if not LocalPlayer():IsListenServerHost() then return end end
		local conflict_help_frame = vgui.Create( "DFrame" )
		conflict_help_frame:SetSize(200, 200)
		conflict_help_frame:Center()
		...
		function conflictbut.DoClick()
			steamworks.ViewFile(2828787576)
		end
	end
end