...
function meta:IsFriendly(target)
	...
	if (self:IsPlayer() and self:IsL4D() and t1 == TEAM_BLU and target:IsNPC() and target:Classify() == CLASS_ZOMBIE) then
		return true
	end
	if (target:IsPlayer() and target:IsL4D() and t2 == TEAM_BLU and self:IsNPC() and self:Classify() == CLASS_ZOMBIE) then
		return true
	end
	...
	return (self:HasNPCFlag(NPC_ALWAYSFRIENDLY) or
			...
			(t1==TEAM_RED or t1==TEAM_BLU or t1==TF_TEAM_PVE_INVADERS or t1==TEAM_INFECTED) and t1==t2)
end

function meta:CanDamage(target)
	if (self:IsPlayer() and self:IsL4D() and self:EntityTeam() == TEAM_BLU and target:IsNPC() and target:Classify() == CLASS_ZOMBIE) then
		return false
	end
