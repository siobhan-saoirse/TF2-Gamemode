...
hook.Add("CreateMove", "IncapMove", function(cmd)
	if LocalPlayer():GetNWBool("Incapped") then
		cmd:ClearMovement()
		return true
	end
end)
...
hook.Add("CalcView", "TFCalcView", function(pl, pos, ang, fov)
	...
	if pl.FirstReality and pl:Alive() then
		if pl:IsHL2() then
			...
			pl:ManipulateBoneScale(pl:LookupBone("bip_neck"), Vector(0,0,0))
	...
	if pl.NextEndThirdperson then
		if CurTime()>pl.NextEndThirdperson then
			...
			if pl.CurrentView and not tf_thirdperson:GetBool() then -- stupid bug fix
				...
				pl.CurrentView.distance = Lerp((pl.NextEndThirdperson - CurTime())/ThirdpersonEndDelay, 1, pl.TargetView.distance)
				if pl:IsHL2() then
...
function EndThirdperson(immediate)
	if immediate then
		...
		LocalPlayer().NextEndThirdperson = CurTime() + 0.5
	end
end
