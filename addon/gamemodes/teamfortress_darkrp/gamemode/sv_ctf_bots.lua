...
hook.Add("PlayerSpawn", "LeadBot_S_PlayerSpawn", function(bot)
	if (IsValid(bot)) then
		if bot.LeadBot and bot:GetPlayerClass() == "gmodplayer" then
			bot:SelectWeapon(tostring(bot:GetWeapons()[math.random(3,4)]:GetClass()))
		elseif bot.LeadBot then
...
hook.Add("SetupMove", "LeadBot_Control", function(bot, mv, cmd)
	...
	elseif IsValid(bot.TargetEnt) then
		...
		if (bot:IsL4D()) then
			if IsValid(bot:GetActiveWeapon()) and bot:GetPos():Distance(bot.TargetEnt:GetPos()) < 70 and bot:GetPlayerClass() != "medic" then 
				if (bot:GetActiveWeapon().HoldType != "MELEE" && bot:GetActiveWeapon().HoldType != "MELEE_ALLCLASS") then
	...
	local controller = bot.ControllerBot
	local curgoal = bot.LastPath[bot.CurSegment]
    if bot:GetVelocity():Length2DSqr() <= 225 and bot.TargetEnt then
	...
	if (!IsValid(bot.TargetEnt) or (bot.TargetEnt:IsPlayer() and bot.TargetEnt:Alive())) then
	...
	if IsValid(bot.TargetEnt) then
		...
		if bot:GetPlayerClass() == "demoknight" or bot:GetPlayerClass() == "giantdemoknight" then
	...
	------------------------------
		...
		if IsValid(bot.TargetEnt) and bot.TargetEnt:Health() > 1 and bot:GetNWBool("Taunting",false) != true then
			...
			if (bot:IsL4D()) then
				bot:SetEyeAngles(LerpAngle(FrameTime() * 2, bot:EyeAngles(), (shouldvegoneforthehead - bot:GetShootPos()):Angle()))
			else
		...
		elseif !IsValid(bot.TargetEnt) and bot:GetPos():Distance(curgoal.pos) > 20 and bot:GetNWBool("Taunting",false) != true then
			...
			if (bot:IsL4D()) then
				bot:SetEyeAngles(LerpAngle(FrameTime() * 3, bot:EyeAngles(), ((curgoal.pos + Vector(0, 15, 65)) - bot:GetShootPos()):Angle()))
			else
				bot:SetEyeAngles(LerpAngle(FrameTime() * 12 * lerp, bot:EyeAngles(), ((curgoal.pos + Vector(0, 15, 65)) - bot:GetShootPos()):Angle()))
			end
		end
	...
	if bot.LKBot  then
		...
		if ( bot:IsBot() and !BotCanTarget and !IsValid(bot.TargetEnt) ) then 
			...
			for k,v in ipairs(ents.FindInSphere(bot:GetPos(), 1200)) do
				if (v:IsPlayer() and v:IsBot() and v:EntIndex() != bot:EntIndex()) then
					if (v.LeadBot and (v:GetPlayerClass() == "medic") and v:IsFriendly(bot)) then
		...
		for k,v in ipairs(player.GetAll()) do 
			if v:IsTFPlayer() and bot:GetPos():Distance(v:GetPos()) < 200 && (!v:IsFriendly(bot)) then
				if bot:IsL4D() then
					if (v:EntIndex() == bot:GetEyeTrace().Entity:EntIndex() or (v:GetPos():Distance(bot:GetEyeTrace().HitPos) < 100 and v:Alive())) then
						buttons = buttons + IN_ATTACK
					end
				end
			end
		...
		if IsValid(bot.TargetEnt) then
			...
			for k,v in ipairs(ents.FindByClass("rpg_missile")) do
				...
				if bot:GetPos():Distance(bot.TargetEnt:GetPos()) < 1100 and bot.TargetEnt:Health() > 1 then
					if bot:GetNWBool("Taunting") == false then
						...
						elseif bot:GetPlayerClass() == "gmodplayer" and math.random(1,120) == 1 then 	
							bot:SelectWeapon(tostring(bot:GetWeapons()[math.random(3,4)]:GetClass()))
							if bot:GetPos():Distance(bot.TargetEnt:GetPos()) < 1100 then
								buttons = buttons + IN_ATTACK
							end
						...
						elseif bot:GetPlayerClass() == "demoman" then 		
							...
							if bot:IsL4D() and bot:GetPos():Distance(bot.TargetEnt:GetPos()) < 180 and bot:GetPlayerClass() != "tank" then
							...
							elseif bot:IsL4D() and bot:GetPos():Distance(bot.TargetEnt:GetPos()) < 370 and bot:GetPlayerClass() == "tank" and math.random(1,36) == 1 then
								if (CurTime() > bot:GetActiveWeapon():GetNextSecondaryFire()) then
									buttons = buttons + IN_ATTACK2
								end
							elseif bot:IsL4D() and bot:GetPos():Distance(bot.TargetEnt:GetPos()) < 180 and bot:GetPlayerClass() == "tank" then
								buttons = buttons + IN_ATTACK
							elseif !bot:IsL4D() then
								buttons = buttons + IN_ATTACK
							end
		...
		--[[for k,v in ipairs(player.GetAll()) do
			...
			if (!GetConVar("nb_blind"):GetBool() and IsValid(bot.TargetEnt)) then
				...
				elseif bot:GetPos():Distance(bot.ControllerBot.PosGen) < 100 and ((bot.TargetEnt:IsPlayer() and bot.TargetEnt:GetPlayerClass() != "boomer") or (bot.TargetEnt:IsTFPlayer() and !bot.TargetEnt:IsPlayer())) then
					...
					if (bot:GetActiveWeapon().HoldType != "MELEE" and bot:GetActiveWeapon().HoldType != "MELEE_ALLCLASS") then
						cmd:SetForwardMove(-1200)
						if (bot:GetEyeTrace().Entity == bot.TargetEnt) then
							cmd:SetSideMove(-250)
						else
		...
		if bot:GetVelocity():Length2DSqr() <= 225 then
			...
			elseif controller.nextStuckJump < CurTime() then
				if !bot:Crouching() then
					controller.NextJump = 0
					controller.NextDuck = CurTime() + math.Rand(0.3,1)
				end
				controller.nextStuckJump = CurTime() + math.Rand(1, 2)
			end
		end
...
hook.Add("PostPlayerDeath", "leadbot_respawn", function(bot)
	if (IsValid(bot)) then
		...
		timer.Simple(6.5, function() if IsValid(bot) and bot.LKBot and !bot:Alive() then bot:Spawn() end end)
	end
end)
