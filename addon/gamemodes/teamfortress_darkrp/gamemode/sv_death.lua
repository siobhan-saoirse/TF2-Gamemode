...
function GM:DoPlayerDeath(ply, attacker, dmginfo)
	...
	if (!ply:IsL4D() and !ply:IsHL2() and !dmginfo:IsDamageType(DMG_BLAST) && !dmginfo:IsDamageType(DMG_ALWAYSGIB) and ply:GetPlayerClass() != "boomer" && ply:GetPlayerClass() != "tank" and !(string.find(ply:GetModel(),"bot_") and string.find(ply:GetModel(),"_boss") or ply:GetModelScale() > 1.0)) then
	...
	elseif ((ply:IsL4D() or ply:IsHL2()) and ply:GetPlayerClass() != "boomer" && ply:GetPlayerClass() != "tank") then
		timer.Simple(0.1, function()
			if ply:GetRagdollEntity():IsValid() then
				ply:GetRagdollEntity():Remove()
			end
	...
	elseif (ply:GetPlayerClass() == "tank") then
	...
	if (attacker:IsTFPlayer() and attacker != ply) then
	...
	if (string.find(ply:GetModel(),"bot_") and string.find(ply:GetModel(),"_boss") or ply:GetModelScale() > 1.0) then
		ply:GibBreakClient( dmginfo:GetDamageForce() )
	end
	if (ply:GetPlayerClass() == "spitter") then
