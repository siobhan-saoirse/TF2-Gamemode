...
local health_bg = surface.GetTextureID("hud/armor_heavy_bg")
local health_color = surface.GetTextureID("hud/armor_gradient_red")
local health_over_bg = surface.GetTextureID("hud/health_over_bg")
local health_dead = surface.GetTextureID("hud/health_dead")
 
local delta_item_x = 38
local delta_item_start_y = 90 
local delta_item_end_y = 30
...
if HudArmorAccount then HudArmorAccount:Remove() end
HudArmorAccount = vgui.CreateFromTable(vgui.RegisterTable(PANEL, "DPanel"))
