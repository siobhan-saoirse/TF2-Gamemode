...
["jarate_duration"] = {
	pre_damage = function(v,ent,hitgroup,dmginfo)
		...
		local att = dmginfo:GetAttacker()
		for k,v in ipairs(ents.FindInSphere(inf:GetPos(), 180)) do
			if v:IsTFPlayer() and v~=att and v:CanReceiveCrits() and att:IsValidEnemy(v) then
				v:AddPlayerState(PLAYERSTATE_JARATED, true)
				timer.Simple(0, function() if IsValid(v) then v:AddPlayerState(PLAYERSTATE_JARATED, true) end end)
				
				v.NextEndJarate = CurTime() + v
