...
function GM:RollCritical(pl)
	...
	-- THAT IS DISGUSTING, WHY WOULD YOU DO THAT
	if pl:HasPlayerState(PLAYERSTATE_PUKEDON) then
		pl:SetNWBool("NextShotIsCritical", false)
		return
	end
...
function GM:Tick()
	for k,v in ipairs(ents.GetAll()) do
		if (v:IsNPC() and string.find(v:GetModel(),"cpthazama/l4d")) then
			
			for _,ply in ipairs(ents.FindInSphere(v:GetPos(), 1024)) do
			...
			if (math.random(1,80) == 1) then
				for i=1,2 do
					local zombies = {
						...
						"npc_vj_l4d_com_female",
						"npc_vj_l4d_com_female",
						"npc_vj_l4d_com_m_airport",
						"npc_vj_l4d_com_m_airport",
						"npc_vj_l4d_com_m_airport",
						"npc_vj_l4d_com_m_police",
						"npc_vj_l4d_com_m_soldier",
						"npc_vj_l4d_com_m_hospital",
						"npc_vj_l4d_com_m_worker",
						"npc_vj_l4d_com_m_riot",
						"npc_vj_l4d_com_m_ceda",
						"npc_vj_l4d_com_m_clown"
					}
					local zombie = ents.Create(table.Random(zombies))
					zombie:SetPos(ply:GetPos() + Vector(math.random(-1200,1200),math.random(-1200,1200),30))
					zombie:Spawn()
					zombie:Activate()
					zombie:SetEnemy(ply)
					zombie:SetTarget(ply)
...
function GM:Think()
	...
	for _,pl in ipairs(player.GetAll()) do
		if (IsValid(pl.TargetEnt) and (pl.TargetEnt:EntIndex() == pl:EntIndex() or pl.TargetEnt:GetClass() == "ctf_bot_navigator")) then
			pl.TargetEnt = nil
		end
		if (pl.TFBot) then
			if (IsValid(pl:GetActiveWeapon())) then
				if (pl:GetActiveWeapon():Ammo1() < 1 and !pl:GetActiveWeapon().IsMeleeWeapon and pl:GetActiveWeapon().Slot == 0) then
					pl:SelectWeapon(pl:GetWeapons()[2]:GetClass())
				elseif (pl:GetActiveWeapon():Ammo1() < 1 and pl:GetActiveWeapon().Slot == 1) then
					pl:SelectWeapon(pl:GetWeapons()[3]:GetClass())
				end
			end
		end
		...
		for k,v in ipairs(ents.FindInSphere(pl:GetShootPos() * 1.05,80 * pl:GetModelScale() * 1.01)) do
			...
			if (pl:IsPlayer() and !pl:IsHL2() and IsValid(pl:GetActiveWeapon()) and pl:GetActiveWeapon():GetItemData() and pl:GetActiveWeapon():GetItemData().item_name == "#TF_Minigun_Deflector" and (string.find(v:GetClass(), "tf_projectile_") or v:GetClass() == "prop_combine_ball" or v:GetClass() == "rpg_missile" or v:GetCollisionGroup() == COLLISION_GROUP_PROJECTILE) and pl:KeyDown(IN_ATTACK)) then
