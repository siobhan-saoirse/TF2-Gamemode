...
local classtbl4d = {"tank","boomer","boomer","boomer","jockey","charger","charger","spitter","spitter","smoker","hunter"}
...
hook.Add("PlayerHurt", "LeadBot_Death", function(ply, bot, hp, dmg)
    if bot.TFBot then
        ...
        controller.LookAt = ((bot:GetPos() + VectorRand() * 128) - bot:GetPos()):Angle()
    end
end)
...
hook.Add("SetupMove", "LeadBot_Control", function(bot, mv, cmd)
	...
	if (!bot:GetNWBool("Taunting") && !GetConVar("nb_blind"):GetBool() and !IsValid(bot.TargetEnt)) then
	...
	for k, v in pairs(ents.GetAll()) do
		if (!IsValid(bot.TargetEnt)) then
			if v:IsNPC() and v:GetPos():Distance(bot:GetPos()) < 1200 and !IsValid(bot.TargetEnt) then
	...
	elseif IsValid(bot.TargetEnt) then
		...
		-- TODO: find a random spot rather than trying to back up into what could just be a wall
			...
			elseif IsValid(bot:GetActiveWeapon()) and bot:GetPos():Distance(bot.TargetEnt:GetPos()) < 250 and bot:GetPlayerClass() == "medic" and bot.TargetEnt:IsFriendly(bot) then 
				mv:SetForwardSpeed(0)
	...
	if (!GetConVar("nb_blind"):GetBool()) then
    ...
    if bot:GetVelocity():Length2DSqr() <= 225 then
        ...
        elseif controller.nextStuckJump < CurTime() and controller.PosGen then
            if !bot:Crouching() then
	...
	if (bot:GetPlayerClass() == "samuraidemo") then
		if controller.nextStuckJump < CurTime() and controller.PosGen then
			if !bot:Crouching() then
				controller.NextJump = 0
			end
	...
	local controller = bot.ControllerBot
    local mva = ((curgoal.pos + Vector(0, 0, 72)) - bot:GetShootPos()):Angle()

 	if (!GetConVar("nb_blind"):GetBool() and !bot:GetNWBool("Taunting",false)) then
	...
	if IsValid(bot.TargetEnt) and !IsValid(bot.Teleporter) and bot.TargetEnt:Health() > 0 and bot:GetNWBool("Taunting",false) != true then
		...
		elseif bot.Difficulty == 3 then
			...
			if (bot:GetPlayerClass() == "soldier" and (bot:GetActiveWeapon().HoldType == "PRIMARY" or bot:GetActiveWeapon().HoldType == "PRIMARY2")) then
				bot:SetEyeAngles(LerpAngle(FrameTime() * 12 * lerp, bot:EyeAngles(), (shouldvegoneforthehead - bot:GetShootPos() - Vector(0,0,40)):Angle()))
			else
				bot:SetEyeAngles(LerpAngle(FrameTime() * 12 * lerp, bot:EyeAngles(), (shouldvegoneforthehead - bot:GetShootPos()):Angle()))
			end
		return
	elseif !IsValid(bot.TargetEnt) and bot:GetPos():Distance(curgoal.pos) > 20 and bot:GetNWBool("Taunting",false) != true then
			if (IsValid(bot.TargetEnt) and bot.TargetEnt:Health() < 0) then 
				bot.TargetEnt = nil
			end
			local lerp = 1.2
			...
			elseif bot.Difficulty == 3 then
				...
				if (bot:IsHL2()) then
					bot:SetEyeAngles(LerpAngle(FrameTime() * 12 * lerp, bot:EyeAngles(), ((curgoal.pos + Vector(0, 15, 67) * bot:GetModelScale()) - bot:GetShootPos()):Angle()))
				else
					bot:SetEyeAngles(LerpAngle(FrameTime() * 12 * lerp, bot:EyeAngles(), ((curgoal.pos + Vector(0, 15, 72) * bot:GetModelScale()) - bot:GetShootPos()):Angle()))
				end
		end
...
hook.Add("StartCommand", "leadbot_control", function(bot, cmd)
	if bot.TFBot  then
		...
		for k,v in ipairs(ents.FindInSphere(bot:GetPos(),150)) do
			if (IsValid(v) and v:IsPlayer() and v:Team() == bot:Team() and v:EntIndex() != bot:EntIndex() and (bot:GetPlayerClass() != "medic" and bot:GetPlayerClass() != "engineer")) then
				...
				avoidVector = avoidVector + ( Vector(1,1,1) - ( between2 / 150 ) ) * between
				...
				if v:IsPlayer() and v:EntIndex() != bot:EntIndex() and v:GetPos():Distance(bot:GetPos()) < 1200 and !IsValid(bot.TargetEnt) then
					if (!v:IsFriendly(bot)) then -- TODO: find a better way to do this
						...
						for _,trgt in ipairs(ents.FindInSphere(trace.HitPos, 230)) do
							if (trgt == bot.TargetEnt and !trgt:IsFlagSet(FL_NOTARGET)) then
								bot.TargetEnt = trgt
							end
						end
					end
		...
		if bot:Armor() < bot:GetMaxArmor() / 3 then
			for k,v in ipairs(ents.FindInSphere(bot:GetPos(), 1200)) do
				if (v:IsPlayer() and v:IsBot() and v:EntIndex() != bot:EntIndex()) then
					if (v.TFBot and (v:GetPlayerClass() == "engineer") and v:IsFriendly(bot)) then
						if (!IsValid(v.TargetEnt)) then
							if (bot:Armor() < bot:GetMaxArmor() - 1) then
								v.TargetEnt = bot
							else
								v.TargetEnt = nil
							end
		...
		if bot:Health() < bot:GetMaxHealth() / 3 then
			for k,v in ipairs(ents.FindInSphere(bot:GetPos(), 2200)) do
				if (v:IsPlayer() and v:IsBot() and v:EntIndex() != bot:EntIndex()) then
					if (v.TFBot and (v:GetPlayerClass() == "medic") and v:IsFriendly(bot)) then
						if (!IsValid(v.TargetEnt)) then
		...
		for k,v in ipairs(ents.FindByClass("prop_physics")) do 
			if bot:GetPos():Distance(v:GetPos()) < 450 then
				if bot:GetPlayerClass() == "tank" then
		...
		if IsValid(bot.TargetEnt) then
			...
			if (bot.TargetEnt:Health() > 0) then
				...
				if bot:GetPlayerClass() == "hunter" then
			...
			for k,v in ipairs(ents.FindByClass("rpg_missile")) do
				...
				if bot:GetPos():Distance(bot.TargetEnt:GetPos()) < 1100 and bot.TargetEnt:Health() > 0 then
					if bot:GetNWBool("Taunting") == false then
						...
						elseif bot:GetPlayerClass() == "sniper" then 		
							if bot:GetPos():Distance(bot.TargetEnt:GetPos()) < 350 then
								...
								if bot:GetPos():Distance(bot.TargetEnt:GetPos()) < 1100 then
									buttons = buttons + IN_ATTACK
									if (bot.TargetEnt:IsPlayer() and bot.TargetEnt:GetEyeTrace().Entity == bot) then
										cmd:SetSideMove(-250)
									else
										cmd:SetSideMove(250)
									end
						...
						elseif bot:GetPlayerClass() == "spy" then 		
							if bot:GetPos():Distance(bot.TargetEnt:GetPos()) < 350 and !bot.TargetEnt:IsBuilding() then
							...
							elseif bot:GetPos():Distance(bot.TargetEnt:GetPos()) < 350 and bot.TargetEnt:IsBuilding() then
								if (bot:GetWeapons()[5]:EntIndex() != bot:GetActiveWeapon():EntIndex()) then
									bot:SelectWeapon(bot:GetWeapons()[5]:GetClass())
								end
								buttons = buttons + IN_ATTACK
							elseif bot:GetPos():Distance(bot.TargetEnt:GetPos()) > 350 then
						...
						elseif bot:GetPlayerClass() == "demoman" and !string.find(bot:GetModel(),"bot_") then 		
							if bot:GetPos():Distance(bot.TargetEnt:GetPos()) < 200 and bot:GetPos():Distance(bot.TargetEnt:GetPos()) > 120 then
								if (IsValid(bot:GetWeapons()[3])) then
									bot:SelectWeapon(bot:GetWeapons()[3]:GetClass())
									if bot:GetPos():Distance(bot.TargetEnt:GetPos()) < 250 then
										if bot:GetPos():Distance(bot.TargetEnt:GetPos()) < 90 then
											buttons = buttons + IN_DUCK + IN_ATTACK
										else
											buttons = buttons + IN_ATTACK
										end
										if (bot.TargetEnt:IsPlayer() and bot.TargetEnt:GetEyeTrace().Entity == bot) then
								...
								elseif (IsValid(bot:GetWeapons()[2]) and !IsValid(bot:GetWeapons()[3])) then
									bot:SelectWeapon(bot:GetWeapons()[2]:GetClass())
									if bot:GetPos():Distance(bot.TargetEnt:GetPos()) < 250 then
							...
							elseif bot:GetPos():Distance(bot.TargetEnt:GetPos()) < 120 then
								if (IsValid(bot:GetWeapons()[3])) then
									bot:SelectWeapon(bot:GetWeapons()[3]:GetClass())
									if bot:GetPos():Distance(bot.TargetEnt:GetPos()) < 1000 then
										buttons = buttons + IN_ATTACK
							...
							elseif bot:GetPos():Distance(bot.TargetEnt:GetPos()) > 420 then
						...
						elseif bot:GetPlayerClass() == "gmodplayer" then 	
							...
							if bot:GetActiveWeapon().IsL4DWeapon then 		
								if bot:GetPos():Distance(bot.TargetEnt:GetPos()) < 120 then
									buttons = buttons + IN_ATTACK2
									local shouldvegoneforthehead = bot.TargetEnt:EyePos()
									local group = math.random(0, bot.TargetEnt:GetHitBoxGroupCount() - 1)
									local bone = bot.TargetEnt:GetHitBoxBone(math.random(0, bot.TargetEnt:GetHitBoxCount(group) - 1), group) or 0
									shouldvegoneforthehead = bot.TargetEnt:GetBonePosition(bone)
									bot:SetEyeAngles(LerpAngle(FrameTime() * 15, bot:EyeAngles(), (shouldvegoneforthehead - bot:GetShootPos()):Angle()))
								end
							end
							if bot:GetPos():Distance(bot.TargetEnt:GetPos()) < 1100 and math.random(1,3) == 1 then
								if (bot:Visible(bot.TargetEnt) and bot:GetNWBool("Taunting",false) != true) then
									buttons = buttons + IN_ATTACK
								end
							end 
						elseif bot:GetPlayerClass() == "engineer" and bot.TargetEnt:EntityTeam() == bot:Team() then 	
							local shouldvegoneforthehead = bot.TargetEnt:EyePos()
							local group = math.random(0, bot.TargetEnt:GetHitBoxGroupCount() - 1)
							local bone = bot.TargetEnt:GetHitBoxBone(math.random(0, bot.TargetEnt:GetHitBoxCount(group) - 1), group) or 0
							shouldvegoneforthehead = bot.TargetEnt:GetBonePosition(bone)
							bot:SetEyeAngles(LerpAngle(FrameTime() * 12, bot:EyeAngles(), (shouldvegoneforthehead - bot:GetShootPos()):Angle()))
							if (bot:GetWeapons()[3]:EntIndex() != bot:GetActiveWeapon():EntIndex()) then
								bot:SelectWeapon(tostring(bot:GetWeapons()[3]:GetClass()))
							end
							if (bot.TargetEnt:IsPlayer() and bot.TargetEnt:GetEyeTrace().Entity == bot) then
								cmd:SetForwardMove(1200)
		...
		--[[for k,v in ipairs(player.GetAll()) do
			...
			if (!GetConVar("nb_blind"):GetBool() and IsValid(bot.TargetEnt)) then
				if bot:GetPos():Distance(bot.ControllerBot.PosGen) > 150 then
					--cmd:SetForwardMove( 250 )
					if (!GetConVar("nb_blind"):GetBool() and bot:GetNWBool("Taunting",false) != true) then
						cmd:SetForwardMove(1200)
					end
				elseif bot:GetPos():Distance(bot.ControllerBot.PosGen) < 100 and ((bot.TargetEnt:IsPlayer() and bot.TargetEnt:GetPlayerClass() != "boomer") or (bot.TargetEnt:IsTFPlayer() and !bot.TargetEnt:IsPlayer())) then
				...
				elseif bot:GetPos():Distance(bot.ControllerBot.PosGen) < 800 and bot.TargetEnt:IsPlayer() and bot.TargetEnt:GetPlayerClass() == "boomer" then
					--cmd:SetForwardMove( -130 )
					cmd:SetForwardMove(-1200)
				end
			end
		...
		if bot:GetVelocity():Length2DSqr() <= bot:GetClassSpeed() then
			if controller.NextCenter < CurTime() then
				controller.strafeAngle = ((controller.strafeAngle == 1 and 2) or 1)
				controller.NextCenter = CurTime() + math.Rand(0.3, 0.65)
...
concommand.Add("tf_bot_add", function(ply, _, _, args) if (game.SinglePlayer()) then print("Doesn't work in Singleplayer!") end if ply:IsAdmin() or ply:IsSuperAdmin() then LeadBot_S_Add(args[1]) end end)
concommand.Add("z_spawn", function(ply, _, _, args) 
	if ply:IsAdmin() or ply:IsSuperAdmin() then 
		...
		if (game.SinglePlayer()) then 
			...
			if (args == "smoker" || args == "boomer" || args == "hunter" || args == "tank" || args == "charger" || args == "jockey" || args == "spitter") then
