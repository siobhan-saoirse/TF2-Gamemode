...
function ENT:Load()
	self:Clear()
	if not file.Exists("pf2/landmark.txt", "DATA") then return 1 end
	local ok, t = pcall(util.JSONToTable, file.Read("pf2/landmark.txt", "DATA"))
	
	if not ok then
		ErrorNoHalt(t)
		file.Delete("pf2/landmark.txt")
		return 2
	end
...
function ENT:Save()
	...
	file.Write("pf2/landmark.txt", util.TableToJSON(t))
	file.Write("pf2/landmark2.txt", util.TableToJSON(t))
	
	file.Append("pf2/log.txt", Format("Saving landmark data, map: %s\n", self.Map))
end
