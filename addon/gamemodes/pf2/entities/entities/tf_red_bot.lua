	...
	timer.Simple(0.1, function()
		if IsValid(bot) then
			...
			if (bot:GetPlayerClass() == "gmodplayer") then
				...
				bot:SetPlayerColor(Vector(math.Rand(0,255) / 255,math.Rand(0,255) / 255,math.Rand(0,255) / 255))
				bot:StripWeapons()
...
function RandomWeapon2(ply, wepslot)
	...
	for k, v in pairs(weps) do
		if v and istable(v) and isstring(wepslot) and v["name"] and v["item_slot"] == wepslot and v["used_by_classes"] and v["used_by_classes"][class] and !string.find(v["name"], "Jumper") and v["craft_class"] == "weapon" then
...
function RandomCosmetic(ply, wepslot)
	...
	local validweapons = {}
	for k, v in pairs(weps) do
		if v and istable(v) and isstring(wepslot) and v["name"] and v["item_slot"] == wepslot and v["used_by_classes"] and v["used_by_classes"][class] and !string.find(v["name"], "Medal") and v["equip_region"] != "medal" and (v["item_class"] == "tf_wearable" || !IsValid(v["item_class"]) ) then
			table.insert(validweapons, v["name"])
		end
	end
