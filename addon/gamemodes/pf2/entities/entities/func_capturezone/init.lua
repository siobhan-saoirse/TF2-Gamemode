		...
		if v.Carrier==ply and self.Team ~= v.te and v.Prop:GetPos():Distance(ply:GetPos()) <= 100 then
			...
			for _, ply in pairs(player.GetAll()) do
				if ply:Team() ~= v.TeamNum then
					if game.GetMap() == "mvm_terroristmission_v7_1" then
						ply:SendLua([[surface.PlaySound("vo/mvm_final_wave_end0"..math.random(1,6)..".wav")]])
					else
						ply:SendLua([[surface.PlaySound("vo/intel_teamcaptured.wav")]])
					end
					...
					ply:SendLua([[surface.PlaySound("vo/intel_enemycaptured.wav")]])
				end
			end
		end
	end
		...
		if v.Carrier==ply and self.Team ~= v.Team then
				...
				timer.Simple(1, function()
					...
					for _, player in ipairs(player.GetAll()) do
						player:SendLua([[surface.PlaySound("vo/mvm_bomb_alerts0"..math.random(8,9)..".wav")]])
					end
				...
				timer.Simple(4, function()
					...
					for _,pl in pairs(player.GetAll()) do
						if pl:Team() == TEAM_RED then
							pl:SendLua([[surface.PlaySound("vo/mvm_wave_lose0"..math.random(1,8)..".wav", 50, 100)]])
						end
					end
					RunConsoleCommand("tf_mvm_wins")
