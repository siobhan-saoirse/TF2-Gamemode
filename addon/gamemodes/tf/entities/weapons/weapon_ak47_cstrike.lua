...
SWEP.Primary.DefaultClip = 12880 -- How much bullets preloaded when spawned
...
function SWEP:Deploy()
	self:SetWeaponHoldType( self.HoldType ) 
		...
		if SERVER then
			self:SendWeaponAnim(vm:GetSequenceActivity(vm:LookupSequence("ak47_draw")))
		end
		timer.Stop("Idle"..self.Owner:EntIndex())
		timer.Stop("Idle2"..self.Owner:EntIndex())
		local vm = self:GetOwner():GetViewModel()
		timer.Create("Idle"..self.Owner:EntIndex(), vm:SequenceDuration(vm:LookupSequence("ak47_draw")) , 1, function()
			self:SendWeaponAnim(vm:GetSequenceActivity(vm:LookupSequence("ak47_idle")))
			timer.Create("Idle2"..self.Owner:EntIndex(), vm:SequenceDuration(vm:LookupSequence("ak47_idle")) , 0, function()
				self:SendWeaponAnim(vm:GetSequenceActivity(vm:LookupSequence("ak47_idle")))
			end)
		end)
	return true
...
function SWEP:PrimaryAttack()
	...
	self.Owner:FireBullets( bullet ) 
		...
		self:SendWeaponAnim(vm:GetSequenceActivity(vm:LookupSequence("ak47_shoot"..math.random(1,3))))
		self:SetNextPrimaryFire( CurTime() + self.Primary.Delay ) 
		timer.Stop("Idle"..self.Owner:EntIndex())
		timer.Stop("Idle2"..self.Owner:EntIndex())
		local vm = self:GetOwner():GetViewModel()
		timer.Create("Idle"..self.Owner:EntIndex(), vm:SequenceDuration(vm:LookupSequence("ak47_shoot"..math.random(1,3))) , 1, function()
			self:SendWeaponAnim(vm:GetSequenceActivity(vm:LookupSequence("ak47_idle")))
			timer.Create("Idle2"..self.Owner:EntIndex(), vm:SequenceDuration(vm:LookupSequence("ak47_idle")) , 0, function()
				self:SendWeaponAnim(vm:GetSequenceActivity(vm:LookupSequence("ak47_idle")))
			end)
...
function SWEP:Reload()
		local vm = self:GetOwner():GetViewModel()
		local reload = "ak47_reload"
		if (self:Clip1() < 1) then
			reload = 'ak47_reload'
			self:DefaultReload(vm:GetSequenceActivity(vm:LookupSequence("ak47_reload")))
		else
			self:DefaultReload(vm:GetSequenceActivity(vm:LookupSequence("ak47_reload")))
		end
		if SERVER then
