...
SWEP.HitDistance = 48
SWEP.ShootSound = Sound("weapons/mac10/mac10-1.wav")
SWEP.ViewModelFlip = true
function SWEP:Deploy()
	self:SetWeaponHoldType( self.HoldType ) 
		local vm = self:GetOwner():GetViewModel()
		if SERVER then
			self:SendWeaponAnim(vm:GetSequenceActivity(vm:LookupSequence("mac10_draw")))
		end
		timer.Stop("Idle"..self.Owner:EntIndex())
		timer.Stop("Idle2"..self.Owner:EntIndex())
		local vm = self:GetOwner():GetViewModel()
		timer.Create("Idle"..self.Owner:EntIndex(), vm:SequenceDuration(vm:LookupSequence("mac10_draw")) , 1, function()
			self:SendWeaponAnim(vm:GetSequenceActivity(vm:LookupSequence("mac10_idle")))
			timer.Create("Idle2"..self.Owner:EntIndex(), vm:SequenceDuration(vm:LookupSequence("mac10_idle")) , 0, function()
				self:SendWeaponAnim(vm:GetSequenceActivity(vm:LookupSequence("mac10_idle")))
			end)
		end)
	return true
...
function SWEP:PrimaryAttack()
	...
	self.Owner:FireBullets( bullet ) 
		...
		self:SendWeaponAnim(vm:GetSequenceActivity(vm:LookupSequence("mac10_shoot"..math.random(1,3))))
		self:SetNextPrimaryFire( CurTime() + self.Primary.Delay ) 
		timer.Stop("Idle"..self.Owner:EntIndex())
		timer.Stop("Idle2"..self.Owner:EntIndex())
		local vm = self:GetOwner():GetViewModel()
		timer.Create("Idle"..self.Owner:EntIndex(), vm:SequenceDuration(vm:LookupSequence("mac10_shoot"..math.random(1,3))) , 1, function()
			self:SendWeaponAnim(vm:GetSequenceActivity(vm:LookupSequence("mac10_idle")))
			timer.Create("Idle2"..self.Owner:EntIndex(), vm:SequenceDuration(vm:LookupSequence("mac10_idle")) , 0, function()
				self:SendWeaponAnim(vm:GetSequenceActivity(vm:LookupSequence("mac10_idle")))
			end)
...
function SWEP:Reload()
		local vm = self:GetOwner():GetViewModel()
		local reload = "mac10_reload"
		if (self:Clip1() < 1) then
			reload = 'mac10_reload'
			self:DefaultReload(vm:GetSequenceActivity(vm:LookupSequence("mac10_reload")))
		else
			self:DefaultReload(vm:GetSequenceActivity(vm:LookupSequence("mac10_reload")))
		end
		if SERVER then
