...
ENT.Spawnable		= false
ENT.Model = "models/bots/boss_bot/boss_cart.mdl"
ENT.AttackDelay = 50
ENT.AttackDamage = 30
...
function ENT:Initialize()
	...
	timer.Simple(0.1, function()
		...
		if SERVER then
			...
			self:EmitSound("mvm/giant_heavy/giant_heavy_loop.wav",65,60,0.2)
...
function ENT:HaveEnemy()
	...
	if ( self:GetEnemy() and IsValid(self:GetEnemy()) ) then
		...
		if ( self:GetRangeTo(self:GetEnemy():GetPos()) > self.LoseTargetDist ) then
			...
			self:StopSound("BossCart.Drive")
			self:EmitSound("plats/bigstop1.wav")
...
function ENT:Think()
	...
	if SERVER then
		...
		self:SetAngles(self:EyeAngles())
		self:ManipulateBoneAngles(self:LookupBone("gun_L"),Angle(0,0,self.EyeAngle.x))
		self:ManipulateBoneAngles(self:LookupBone("gun_R"),Angle(0,0,self.EyeAngle.x))
		self:BodyMoveXY()
	...
	if (IsValid(self:GetEnemy()) and self.Ready) then
		...
		if (self:GetEnemy():GetPos():Distance(self:GetPos()) < self.AttackRange and self:GetEnemy():Health() > 0) then
			if (IsValid(self:GetEnemy()) and (!self.item1AttackDelay or CurTime() > self.item1AttackDelay)) then
				...
				self.item1AttackDelay = CurTime() + 5.0
				timer.Simple(3.5, function()
					self:EmitSound("vehicles/tank_readyfire1.wav",95,100,1)
				end) 
				self:EmitSound("vehicles/tank_turret_start1.wav",95,100,1)
				timer.Simple(0.2, function()
					local bomb = ents.Create("tf_projectile_sentryrocket")
					local aimvec = self:EyePos()
					bomb:SetPos(aimvec + (self.EyeAngle:Forward() * 100) + Vector(64,0,0))
					-- Set the angles to the player'e eye angles. Then spawn it.
					self:EmitSound("weapons/stinger_fire1.wav",120,100,1,CHAN_ITEM)
					...
					timer.Simple(0.2, function()
						...
						timer.Simple(0.2, function()
							local bomb = ents.Create("tf_projectile_sentryrocket")
							local aimvec = self:EyePos()
							bomb:SetPos(aimvec + (self.EyeAngle:Forward() * 100) + Vector(64,0,72))
							-- Set the angles to the player'e eye angles. Then spawn it.
								self:EmitSound("weapons/stinger_fire1.wav",120,100,1,CHAN_ITEM)
							bomb:SetAngles( self.EyeAngle )
							bomb:SetOwner(self)
							...
							timer.Simple(0.2, function()
								...
								bomb:Spawn()
								timer.Simple(0.1, function()
									self:EmitSound("vehicles/tank_turret_stop1.wav",95,100,1)
								end)
							end)
						end)
					end)
		...
		if (self:GetEnemy():GetPos():Distance(self:GetPos()) < 100 and self:GetEnemy():Health() > 0) then
			...
			self.loco:SetDesiredSpeed( 0 )		-- Walk speed
			self.loco:SetAcceleration(200)
...
list.Set( "NPC", "boss_cart", {
	Name = "Undentified Machine Type 2",
	Class = "boss_cart",
	Category = "TF2: Raid"
})