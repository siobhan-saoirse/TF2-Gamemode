...
ENT.Spawnable = false
ENT.AdminOnly = true
ENT.PrintName		= "Resonance Cascade"
ENT.Category		= "Civillian 2 Misc"

list.Set( "NPC", "mob_xen_director", {
	Name = ENT.PrintName,
	Class = "mob_xen_director",
	...
	while (true) do 
		...
		if (pos) then
			...
			if (math.random(1,7) == 1) then 
					self:EmitSound("debris/beamstart2.wav",95)
				util.ScreenShake( self:GetPos(), 5, 5, 1, 800 )
				timer.Simple(0.5, function()
				
					local cmb = table.Random(combine)
				self:EmitSound(table.Random({"debris/beamstart7.wav"}),95)
				util.ScreenShake( self:GetPos(), 5, 5, 2, 800 )
							...
							bot:SetPos(self:GetPos() + Vector(0,0,40))
							bot:Spawn()
							bot:Activate()
