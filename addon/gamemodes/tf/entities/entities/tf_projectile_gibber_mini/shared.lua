...
ENT.Model = "models/weapons/w_models/w_bfg_bomblet.mdl"
ENT.Model2 = "models/weapons/w_models/w_stickybomb2.mdl"

ENT.ExplosionSound = Sound("Weapon_Airstrike.Explosion")
		...
		if data.Speed > 50 and data.DeltaTime > 0.2 then
			...
			for k,v in ipairs(ents.FindInSphere(self:GetPos(), 100)) do
				if v:GetClass() == "npc_metropolice" then
					EmitSentence( "METROPOLICE_DANGER_GREN"..math.random(0,2), v:GetPos(), 0.5, CHAN_VOICE, 1, 75, 0, 100 )
				elseif v:GetClass() == "npc_combine_s" then
					EmitSentence( "COMBINE_GREN"..math.random(0,1), v:GetPos(), 1, CHAN_VOICE, 0.5, 75, 0, 100 )
				elseif v:GetClass() == "npc_sniper" then
					EmitSentence( "METROPOLICE_DANGER_VEHICLE0", v:GetPos(), 1, CHAN_VOICE, 0.5, 75, 0, 100 )
				end
			end
		end
