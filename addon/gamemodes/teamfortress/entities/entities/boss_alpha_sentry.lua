...
ENT.Spawnable		= false
ENT.Model = "models/bots/boss_sentry/boss_sentry.mdl"
ENT.AttackDelay = 50
ENT.AttackDamage = 30
...
function ENT:Initialize()
	...
	timer.Simple(0.1, function()
		...
		if SERVER then
			...
			timer.Create("Laugh"..self:EntIndex(), 2, 0, function()
				if (self.Ready && !self:HaveEnemy()) then
					timer.Adjust("Laugh"..self:EntIndex(),2,nil,nil)
					timer.Simple(0.2, function()
						self:EmitSound("Building_Sentrygun.Idle2",95,80)
					end)
					self:EmitSound("Building_Sentrygun.Idle2",95,80)
				end
			end)
...
hook.Add("EntityTakeDamage","RemoveITFromitem1Hit",function(target,dmginfo)
	...
	if (target:IsTFPlayer() and !target:IsNextBot() and (dmginfo:IsDamageType(DMG_CLUB) or dmginfo:IsDamageType(DMG_SLASH)) and att.IsITFromHHH) then
		...
		for k,v in ipairs(ents.FindByClass("boss_alpha_sentry")) do
			if (IsValid(v)) then
				v:SetEnemy(target)
			end
		end
...
function ENT:FindEnemy()
	...
	for k,v in ipairs( _ents ) do
		if ( ( v:IsTFPlayer() and !v:IsNextBot()) and GAMEMODE:EntityTeam(v) != TEAM_SPECTATOR and GAMEMODE:EntityTeam(v) != TEAM_FRIENDLY and v:Health() > 1 and !v:IsFlagSet(FL_NOTARGET) ) then
			...
			self:EmitSound("weapons/sentry_spot.wav",75,100)
			if (v:IsPlayer()) then

				umsg.Start("NotifySentrySpotted", v)
				umsg.End()
...
function ENT:Think()
	...
	if SERVER then
		...
		if (self:HaveEnemy()) then
			self:SetAngles(Angle(self.EyeAngle.x,self.EyeAngle.y,0))
		else
			self:SetAngles(self.EyeAngle)
		end
		self:BodyMoveXY()
	...
	if (IsValid(self:GetEnemy()) and self.Ready) then
		...
		if (self:GetEnemy():GetPos():Distance(self:GetPos()) < self.AttackRange and self:GetEnemy():Health() > 0) then
			if (IsValid(self:GetEnemy()) and (!self.item1AttackDelay or CurTime() > self.item1AttackDelay)) then
				...
				self.item1AttackDelay = CurTime() + 0.6
				timer.Simple(0.3, function()
					local bomb = ents.Create("tf_projectile_sentryrocket")
					local aimvec = self:EyePos()
					bomb:SetPos(aimvec + (self.EyeAngle:Forward() * 100) + Vector(36,0,0))
					...
					timer.Simple(0.3, function()
						local bomb = ents.Create("tf_projectile_sentryrocket")
						local aimvec = self:EyePos()
						bomb:SetPos(aimvec + (self.EyeAngle:Forward() * 100) - Vector(32,0,0))
						-- Set the angles to the player'e eye angles. Then spawn it.
						self:EmitSound("Building_Sentrygun.FireRocket")
						bomb:SetAngles( self.EyeAngle )
						bomb:SetOwner(self)
...
function ENT:OnInjured( dmginfo )
	if (dmginfo:IsDamageType(DMG_BULLET) || dmginfo:IsDamageType(DMG_CLUB)) then
		ParticleEffect( "bot_impact_light", dmginfo:GetDamagePosition(), self:GetAngles() )
		self:StopSound("Building_Sentry.Damage")
		self:EmitSound("Building_Sentry.Damage")
	end
end
