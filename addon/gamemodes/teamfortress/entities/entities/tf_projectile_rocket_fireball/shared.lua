...
ENT.ExplosionSound = Sound("misc/halloween/spell_fireball_impact.wav")
...
ENT.BaseSpeed = 1200
ENT.ExplosionRadiusInit = 80
ENT.OwnerDamage = 1

ENT.CritDamageMultiplier = 1.2
...
function ENT:Think()
	...
	for k,v in ipairs(ents.FindInSphere(self:GetPos(), 100)) do
		if v:IsPlayer() then
			v:EmitSound("weapons/fx/nearmiss/dragons_fury_nearmiss.wav")
		end
	end
...
function ENT:DoExplosion(ent)
	...
	if self.Nuke then
		...
		util.BlastDamage(self, owner, self:GetPos(), range*1.5, damage)
		for k,v in ipairs(ents.FindInSphere(self:GetPos(), range)) do
			if v:IsNPC() or v:IsPlayer() and v:Health() >= 1 then
				GAMEMODE:IgniteEntity(v, self, owner, 10)
				v:Ignite(10)
				v:EmitSound("weapons/dragons_fury_impact_pain.wav")
