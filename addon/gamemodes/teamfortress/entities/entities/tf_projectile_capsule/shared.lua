...
function ENT:InitEffects()
	...
	ParticleEffectAttach("spell_fireball_small_red", PATTACH_POINT_FOLLOW, self, self:LookupAttachment("trail"))
...
function ENT:DoExplosion(ent)
	...
	ParticleEffect("spell_fireball_tendril_parent_red", self:GetPos(), self:GetAngles(), self)
	local effect, angle
	...
	if self.Nuke then
		...
		--[[if self.FastRocket then
			...
			self:EmitSound("misc/halloween/spell_fireball_impact.wav", 120)
		--end
