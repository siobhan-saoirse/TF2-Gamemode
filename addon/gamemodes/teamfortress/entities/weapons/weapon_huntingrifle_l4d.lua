...
SWEP.ViewModel = Model( "models/l4d1/v_models/v_huntingrifle.mdl" )
...
SWEP.HitDistance = 48
SWEP.ShootSound = Sound("^l4d1/weapons/hunting_rifle/gunfire/hunting_rifle_fire_1.wav")
...
function SWEP:Deploy()
	...
	if (!self.DeployAfterPickup) then
		...
		if SERVER then
			self:SendWeaponAnim(vm:GetSequenceActivity(vm:LookupSequence("sniper_draw")))
		end
	end
	...
	if (self:Clip1() < 1) then
		...
		if SERVER then
			if (self:Clip1() < 15) then
				self:SendWeaponAnim(vm:GetSequenceActivity(vm:LookupSequence("sniper_reload_empty")))
			end
		end
	else
		...
		if SERVER then
			if (self:Clip1() < 15) then
				self:SendWeaponAnim(vm:GetSequenceActivity(vm:LookupSequence("sniper_reload1")))
			end
		end
	end
	if SERVER then
