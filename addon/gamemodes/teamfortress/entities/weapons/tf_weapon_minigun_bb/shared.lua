...
function SWEP:PrimaryAttack(vampire)
	...
	if self:RollCritical() then
		if not self.Critting or not self.Firing then
			...
			if SERVER then
				ParticleEffectAttach("eject_minigunbrass", PATTACH_POINT_FOLLOW, self.WModel2, self:LookupAttachment("eject_brass"))
			end
			if self.Primary.Delay == 0.06 then
...
function SWEP:Think()
	...
	if SERVER then
		if IsValid(self) then
			self:ManipulateBoneAngles( 1, Angle(0,self.barrelRotation,0) )
		end
	end
