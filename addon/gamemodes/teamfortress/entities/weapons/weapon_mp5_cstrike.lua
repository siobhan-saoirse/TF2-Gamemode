...
SWEP.HitDistance = 48
SWEP.ShootSound = Sound("^weapons/mp5navy/mp5-1.wav")
...
function SWEP:Reload()
		local vm = self:GetOwner():GetViewModel()
		local reload = "reload1"
		if (self:Clip1() < 1) then
			reload = 'reload1'
			self:DefaultReload(vm:GetSequenceActivity(vm:LookupSequence("reload1")))
		else
			self:DefaultReload(vm:GetSequenceActivity(vm:LookupSequence("reload1")))
		end
		if SERVER then
