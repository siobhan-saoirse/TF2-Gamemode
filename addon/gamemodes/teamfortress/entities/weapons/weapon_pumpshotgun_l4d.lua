...
SWEP.ViewModel = Model( "models/l4d1/v_models/v_pumpshotgun.mdl" )
...
SWEP.ReloadTime = 0.5
SWEP.HitDistance = 48
SWEP.ShootSound = Sound("^l4d1/weapons/shotgun/gunfire/shotgun_fire_1.wav")
...
function SWEP:Deploy()
	...
	local deploy = "shotgun_deploy"
	if (!self.DeployAfterPickup) then
		deploy = "shotgun_deploy"
		if SERVER then
			self:SendWeaponAnim(vm:GetSequenceActivity(vm:LookupSequence("shotgun_deploy")))
		end
	...
	timer.Create("Idle"..self.Owner:EntIndex(), vm:SequenceDuration(vm:LookupSequence(deploy)), 1, function()
		self:SendWeaponAnim(vm:GetSequenceActivity(vm:LookupSequence("shotgun_idle")))
		timer.Create("Idle2"..self.Owner:EntIndex(), vm:SequenceDuration(vm:LookupSequence("shotgun_idle")) , 0, function()
			self:SendWeaponAnim(vm:GetSequenceActivity(vm:LookupSequence("shotgun_idle")))
		end)
	end)
	return true
...
function SWEP:Think()
	...
	if self.NextReload and CurTime()>=self.NextReload then
		...
		if self:Clip1()>=self.Primary.ClipSize or self.Owner:GetAmmoCount(self.Primary.Ammo)==0 then
			...
			self.CanInspect = true
				...
				if SERVER then
					if (self.ReloadingEmpty) then
						self:SendWeaponAnim(vm:GetSequenceActivity(vm:LookupSequence("shotgun_reload_empty_finish")))
						local deploy = "shotgun_reload_empty_finish"
						timer.Create("Idle"..self.Owner:EntIndex(), vm:SequenceDuration(vm:LookupSequence(deploy)), 1, function()
							self:SendWeaponAnim(vm:GetSequenceActivity(vm:LookupSequence("shotgun_idle")))
							timer.Create("Idle2"..self.Owner:EntIndex(), vm:SequenceDuration(vm:LookupSequence("shotgun_idle")) , 0, function()
								self:SendWeaponAnim(vm:GetSequenceActivity(vm:LookupSequence("shotgun_idle")))
						...
						local deploy = "shotgun_reload_finish_nopump"
						self:SendWeaponAnim(vm:GetSequenceActivity(vm:LookupSequence("shotgun_reload_finish_nopump")))
						timer.Create("Idle"..self.Owner:EntIndex(), vm:SequenceDuration(vm:LookupSequence(deploy)), 1, function()
							self:SendWeaponAnim(vm:GetSequenceActivity(vm:LookupSequence("shotgun_idle")))
							timer.Create("Idle2"..self.Owner:EntIndex(), vm:SequenceDuration(vm:LookupSequence("shotgun_idle")) , 0, function()
								self:SendWeaponAnim(vm:GetSequenceActivity(vm:LookupSequence("shotgun_idle")))
			...
			if SERVER then
				if (self.ReloadingEmpty) then
					self:SendWeaponAnim(vm:GetSequenceActivity(vm:LookupSequence("shotgun_reload_empty_start")))
				else
					self:SendWeaponAnim(vm:GetSequenceActivity(vm:LookupSequence("shotgun_reload_start")))
				end
			end
...
function SWEP:PrimaryAttack()
	...
	self:SendWeaponAnim(vm:GetSequenceActivity(vm:LookupSequence("shotgun_shoot1")))
	self:SetNextPrimaryFire( CurTime() + self.Primary.Delay ) 
	timer.Stop("Idle"..self.Owner:EntIndex())
	timer.Stop("Idle2"..self.Owner:EntIndex())
	local vm = self:GetOwner():GetViewModel()
	timer.Create("Idle"..self.Owner:EntIndex(), vm:SequenceDuration(vm:LookupSequence("shotgun_shoot1")) , 1, function()
		self:SendWeaponAnim(vm:GetSequenceActivity(vm:LookupSequence("shotgun_idle")))
		timer.Create("Idle2"..self.Owner:EntIndex(), vm:SequenceDuration(vm:LookupSequence("shotgun_idle")) , 0, function()
			self:SendWeaponAnim(vm:GetSequenceActivity(vm:LookupSequence("shotgun_idle")))
		end)
...
function SWEP:SecondaryAttack( right )
	...
	timer.Stop("Idle"..self.Owner:EntIndex())
	timer.Stop("Idle2"..self.Owner:EntIndex())
	timer.Create("Idle"..self.Owner:EntIndex(), vm:SequenceDuration(vm:LookupSequence("shotgun_melee")) , 1, function()
		self:SendWeaponAnim(vm:GetSequenceActivity(vm:LookupSequence("shotgun_idle")))
		timer.Create("Idle2"..self.Owner:EntIndex(), vm:SequenceDuration(vm:LookupSequence("shotgun_idle")) , 0, function()
			self:SendWeaponAnim(vm:GetSequenceActivity(vm:LookupSequence("shotgun_idle")))
		end)
